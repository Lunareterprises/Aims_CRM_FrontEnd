...
const Customers = () => {
  ...
  const Excutivedelete = async () => {
    try {
      ...
      const response = await axios.post(
        'https://lunarsenterprises.com:5016/crm/delete/customer',
        del,
...
{/* 
              ...
              <div className="d-md-flex justify-content-end align-items-center col-md-4">
                <Button className="mb-3 bgaditem" type="submit" onClick={handleShow}>
                  <i className="fa fa-plus" aria-hidden="true"></i> Add New Customer
                </Button>
                ...
                <tbody className="tablehead">
                  {loading ? (
                    ...
                    products?.map((account, index) => (
                      <tr key={account.cu_id}>
                        <td className="tablehead me-3"> {(currentPage - 1) * limit + index + 1}</td>
                        <td>{`${account.cu_salutation} ${account.cu_first_name} ${account.cu_last_name}`}</td>
                        <td>{account.cu_company_name}</td>
                        <td>{account.cu_display_name}</td>
                        <td>{account.cu_email}</td>

                        <td>
                          <Badge bg={account.cu_status === 'active' ? 'info' : 'danger'}>
                            {account.cu_status}
                          </Badge>
                        </td>
                        <td className="btn-deleteview">
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              handleDeleteConfirmation(account.cu_id)
                            }}
                          >
                            <FontAwesomeIcon
                              icon={faTrash}
                              ...
                              onClick={(e) => {
                                ...
                                handleDeleteConfirmation(account.cu_id) // Show modal for deletion
                              }}
                            />
                          </Button>
                        </td>
                        ...
                        <td className="btn-deleteview">
                          ...
                          >
                            <FontAwesomeIcon
                              ...
                              style={{ cursor: 'pointer' }}
                              onClick={() => {
                                Customerview(account.cu_id) // Show modal for deletion
                              }}
                            />
                          </Button>
                        </td>
            ...
            <div className="banking-container">
              ...
              <div className="button-group">
                <button className="connect-button">
                  <a href="/dashboard/Customer/AddCustomers" className="text-white">CREATE NEW CUSTOMER</a>
                </button>
