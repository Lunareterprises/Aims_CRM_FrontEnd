...
const Salesorder = () => {
  ...
  const handleShow = () => {
    Navi('/dashboard/Salesorder/AddSalesorder')
  }
  const Customerview = (so_id) => {
    Navi(`/dashboard/Salesorder/Saleorderview/${so_id}`)
  }
  // Handle Page Change
  ...
  return (
    <>
      {products.length === 0 ? (
        searchTerm ? (
            ...
            <div className="spacemanger ">
              ...
              <div className="d-md-flex justify-content-end align-items-center col-md-4">
                <Button className="mb-3 bgaditem" type="submit" onClick={handleShow}>
                  <i className="fa fa-plus" aria-hidden="true"></i> Add New Sales Order
                </Button>
                ...
                <tbody className="tablehead">
                  {loading ? (
                    ...
                    products?.map((account, index) => (
                      <tr key={account.so_id}>
                        <td className="tablehead me-3"> {(currentPage - 1) * limit + index + 1}</td>
                        <td>{account.so_customer_name}</td>
                        <td>{account.so_sales_order_id}</td>
                        <td>{moment(account.so_order_date).format('DD-MM-YYYY')}</td>
                        <td>{moment(account.so_shipment_date).format('DD-MM-YYYY')}</td>

                        <td>{account.so_customer_notes}</td>

                        <td>{account.so_total_amount}</td>

                        <td>
                          <Badge bg={account.so_status === 'active' ? 'info' : 'danger'}>
                            {account.so_status}
                          </Badge>
                        </td>
                        <td className="btn-deleteview">
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              handleDeleteConfirmation(account.so_id)
                            }}
                          >
                            <FontAwesomeIcon
                              icon={faTrash}
                              ...
                              onClick={(e) => {
                                ...
                                handleDeleteConfirmation(account.so_id) // Show modal for deletion
                              }}
                            />
                          </Button>
                        </td>
                        ...
                        <td className="">
                          ...
                          >
                            <FontAwesomeIcon
                              ...
                              style={{ cursor: 'pointer' }}
                              onClick={() => {
                                Customerview(account.so_id) // Show modal for deletion
                              }}
                            />
                          </Button>
                        </td>
          ...
          <>
            <div className="banking-container">
              <h2>Start Managing Your Sales Activities</h2>
              <p>Create, customize and send professional sales orders to your customers</p>
              <div className="button-group">
                <button className="connect-button">
                  <a href="/dashboard/Salesorder/AddSalesorder" className="text-white">
                    ADD SALE ORDER
                  </a>
                </button>
                {/* <button className="add-manually-button">Add Manually</button> */}
              </div>
              {/* <p className="skip-link">Click here to import customers from file</p> */}
        ...
        <>
          <div className="spacemanger ">
            ...
            <div className="d-md-flex justify-content-end align-items-center col-md-4">
              <Button className="mb-3 bgaditem" type="submit" onClick={handleShow}>
                <i className="fa fa-plus" aria-hidden="true"></i> Add New Sale Order
              </Button>
      ...
      <Modal show={showDeleteModallist} onHide={handleCloseDeleteModal}>
        ...
        <Modal.Body>Are you sure you want to delete this Sale Order?</Modal.Body>
        <Modal.Footer>
