...
function EquityAccountModal({ show, handleClose, onUpdate }) {
  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      try {
        const response = await fetch("/data/equityAccounts.json"); // Update with your JSON file path
        if (!response.ok) {
  ...
  const handleSelectAll = (category, filteredAccounts) => {
    ...
    setLocalSelected((prev) =>
      allSelected ? prev.filter((acc) => !filteredAccounts.includes(acc)) : [...new Set([...prev, ...filteredAccounts])]
    );
  };
      ...
      <Modal.Body className="px-5">
        ...
        {!loading && !error && (
          <>
            ...
            <div className="ms-3">
              {accountsData.map((categoryObj, index) => {
