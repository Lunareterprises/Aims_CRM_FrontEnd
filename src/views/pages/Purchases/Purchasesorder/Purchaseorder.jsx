...
const Purchaseorder = () => {
  ...
  const handleShow = () => {
    Navi('/dashboard/Purchaseorder/AddPurchaseorder')
  }
  const Customerview = (po_id) => {
    Navi(`/dashboard/Purchaseorder/ViewPurchaseorder/${po_id}`)
  }
  // Handle Page Change
  ...
  return (
    <>
                ...
                <tbody className="tablehead">
                  {loading ? (
                    ...
                    products?.map((account, index) => (
                      <tr key={account.po_id}>
                        <td className="tablehead me-3"> {(currentPage - 1) * limit + index + 1}</td>
                      

                        <td>{account.po_vendor_name}</td>
                        <td>{account.po_order_id}</td>

                       

                        <td>{moment(account.po_order_date).format('DD-MM-YYYY')}</td>

                        <td>{account.po_total_amount}</td>

                        <td>
                          <Badge bg={account.po_status === 'active' ? 'info' : 'danger'}>
                            {account.so_status}
                          </Badge>
                        </td>
                        <td className="btn-deleteview">
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              handleDeleteConfirmation(account.po_id)
                            }}
                          >
                            <FontAwesomeIcon
                              icon={faTrash}
                              ...
                              onClick={(e) => {
                                ...
                                handleDeleteConfirmation(account.po_id) // Show modal for deletion
                              }}
                            />
                          </Button>
                        </td>
                        ...
                        <td className="">
                          ...
                          >
                            <FontAwesomeIcon
                              ...
                              style={{ cursor: 'pointer' }}
                              onClick={() => {
                                Customerview(account.po_id) // Show modal for deletion
                              }}
                            />
                          </Button>
                        </td>
      ...
      <h2>Start Managing Your Purchase Activities!</h2>
      <p>
      ...
      <div className="button-group">
        <button className="connect-button"> 
          ...
          <a href='/dashboard/Purchaseorder/AddPurchaseorder' className='text-white'>
                ...
                {loading ? (
                  ...
                  products?.map((account, index) => (
                    <tr key={account.po_id}>
                      ...
                      <td>{moment(account.po_order_date).format('DD-MM-YYYY')}</td>
                      <td>{moment(account.po_delivery_date).format('DD-MM-YYYY')}</td>
                      <td>
                      

                        {account.po_total_amount}
                        

                      </td>

                      <td>
                        <Badge className='text-capitalize' bg={account.po_status === 'active' ? 'info' : 'danger'}>
                          {account.po_status}
                        </Badge>
                      </td>
