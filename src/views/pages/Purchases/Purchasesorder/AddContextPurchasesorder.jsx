...
const POSalesContext = createContext()

export const POSalesProvider = ({ children }) => {
  const user_id = sessionStorage.getItem('user_id')
  const token = sessionStorage.getItem('token')

  // Global state for table data, products, commission, etc.
  const [data, setData] = useState([
    { key: '1', item: 'Select Your Product',ac:'Select Account', quantity: 1, rate: 0, discount: 0, amount: 0 },
  ])
  ...
  const Bankdetailsoducts = async () => {
    try {
      ...
      if (response.data.result === true) {
        const mappedProducts = response.data.list.map((customer) => ({
          value: customer.bd_id,
          label: customer.bd_acc_bank_name,
        }))
        setBankdetails(mappedProducts)
      } else {
        console.error(response.data.message)
        setBankdetails([])
      }
...
export const POuseSales = () => useContext(POSalesContext)
