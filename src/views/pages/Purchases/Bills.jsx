  ...
  const fetchProducts = async (page = 1, search = '') => {
    ...
    try {
      const response = await axios.post(
        'https://lunarsenterprises.com:5016/crm/list/purchase-bills',
        {
          page_no: page,
          limit: limit,
          search: search,
    ...
    <>
      {products.length === 0 ? (
        searchTerm ? (
            ...
            <div className="spacemanger ">
              ...
              <div className="d-md-flex justify-content-end align-items-center col-md-4">
                <Button className="mb-3 bgaditem" type="submit" onClick={handleShow}>
                  <i className="fa fa-plus" aria-hidden="true"></i> Add New Bill
                </Button>
                ...
                <tbody className="tablehead">
                  {loading ? (
                    ...
                    products?.map((account, index) => (
                      <tr key={account.pb_id}>
                        <td className="tablehead me-3"> {(currentPage - 1) * limit + index + 1}</td>
                        <td>{account.pb_vendor_id}</td>

                        <td>{account.pb_vendor_name}</td>
          ...
          <h2>Owe money? It's good to pay bills on time!</h2>
          <p>
          If you've purchased something for your business, and you don't have to repay it immediately, then you can record it as a bill.
          </p>
          <div className="button-group">
            <button className="connect-button">CREATE A BILL INVOICES</button>
