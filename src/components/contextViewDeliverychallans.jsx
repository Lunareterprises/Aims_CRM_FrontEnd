...
const ContextViewDeliverychallansContextview = createContext()

export const ViewDeliverychallansProvider = ({ children }) => {
  ...
  const dc_id = sessionStorage.getItem('dc_id')
...
const [refreshKey, setRefreshKey] = useState(0);
  const [saleid, setsaleid] = useState(dc_id)
 ...
 console.log(updatedData,'antonytest');
  ...
  const fetchSalesOrder = async () => {
    try {
      ...
      if (response.data.result) {
        ...
        setAdjustment(Number(salesOrder.dc_adjustment) || 0);
        setAccountType(salesOrder.so_tds_tcs || 'N/A');
        setTotalAmounts(Number(salesOrder.so_total_amount) || 0);
  
        const formattedItems = salesOrder.contact_persons
          ? salesOrder.contact_persons.map((item, index) => {
  ...
  return (
    <ContextViewDeliverychallansContextview.Provider
      value={{
        productslist,
        showDeleteModallist,
...
export const useViewDeliverychallansview = () => useContext(ContextViewDeliverychallansContextview)
