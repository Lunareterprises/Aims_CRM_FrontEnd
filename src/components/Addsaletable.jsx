...
import { useSales } from '../views/pages/Banking/SalesContext'


const { Option } = Select

const DraggableTableDiv = () => {
...
<InputGroup className="mb-2">
  <Form.Control placeholder="Adjustment" disabled />
  <Form.Control
    type="text" // Change type to "text" to allow "-" symbol
    value={adjustment}
    onChange={(e) => {
      const value = e.target.value;
      if (/^-?\d*$/.test(value)) {
        setAdjustment(value); // Allow only numbers and negative sign
      }
    }}
    onBlur={() => setAdjustment(parseFloat(adjustment) || 0)} // Convert to number on blur
  />
</InputGroup>
...
<InputGroup className="mb-2">
  ...
  <Form.Control
    ...
    onChange={(e) => {
      const value = e.target.value;
      if (/^-?\d*$/.test(value)) {
        setShipping(value); // Allow only numbers and negative sign
      }
    }}
    onBlur={() => setShipping(parseFloat(shipping) || 0)} // Convert to number on blur
  />
</InputGroup>
...
export default DraggableTableDiv
